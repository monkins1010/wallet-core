...
import { Transaction as VerusTransaction } from '../../types';
import { normalizeTransactionObject } from '../../utils';
import { VerusJsonRpcBaseProvider } from './VerusJsonRpcBaseProvider';
import { Block as VerusRpcBlock, FeeOptions, MinedTransaction, ProviderOptions } from './types';

export class VerusJsonRpcProvider extends Chain<VerusJsonRpcBaseProvider> {
    ...
    constructor(options: ProviderOptions, feeProvider?: Fee, feeOptions?: FeeOptions) {
        super(options.network, new VerusJsonRpcBaseProvider(options), feeProvider);
    ...
    private async getParsedTransactionByHash(transactionHash: string, addFees = false): Promise<Transaction<VerusTransaction>> {
    ...
    private async getTransactionFee(tx: VerusTransaction) {
